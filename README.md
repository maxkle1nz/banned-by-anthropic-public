# BANNED BY ANTHROPIC

BANNED BY ANTHROPIC is a public archive of documented cases from builders and users affected by Anthropic enforcement actions.

This repository is the **public institutional face** of the campaign.

Initiated and edited by **Max Kleinschmidt**.

It exists to make the record:
- visible
- citable
- versioned
- harder to dismiss as rumor

## What lives here

- campaign framing and public purpose
- publication methodology
- privacy boundaries
- public snapshots and aggregate stats
- official updates
- press/research reference material

## What does NOT live here

- the intake backend
- private reports or raw exports
- moderation internals
- admin tooling
- infrastructure or secrets

## Principles

- truth over hype
- privacy over spectacle
- public numbers must be explainable
- private cases can count without being exposed
- publication must resist spam, impersonation, and abuse

## Start here

- [Manifesto](docs/manifesto.md)
- [Methodology](docs/methodology.md)
- [Privacy Boundaries](docs/privacy-boundaries.md)
- [Publication Policy](docs/publication-policy.md)
- [FAQ](docs/faq.md)
- [Press Kit](docs/press-kit.md)
- [Timeline](docs/timeline.md)
- [Launch Update](updates/2026-04-12-launch.md)

## Official links

- Submit a case: https://bannedbyanthropic.com/
- Official site: https://bannedbyanthropic.com/
- Press / research: open a GitHub issue in this repository
- Initiator / editor: Max Kleinschmidt

## If this matters

- submit your case through the official site
- cite this repository when referencing the campaign
- star/watch the repository if you want to follow public updates
- open an issue for factual corrections

BANNED BY ANTHROPIC exists to turn scattered private damage into a public record.
